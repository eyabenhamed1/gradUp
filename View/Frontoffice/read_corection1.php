...
<body>
          ...
          <?php if ($corrections && $corrections->rowCount() > 0): ?>
            <?php while($row = $corrections->fetch(PDO::FETCH_ASSOC)): ?>
              <tr>
                ...
                <td>
                  <?php if (!empty($row['image2'])): ?>
                    <button class="pdf-btn" onclick="generateSinglePDF('<?= htmlspecialchars($row['id_exam']) ?>', '<?= htmlspecialchars(addslashes($row['remarque'])) ?>', '<?= htmlspecialchars(addslashes($row['note'])) ?>')">
  ...
  <!-- Scripts -->
  <script>
    async function generateSinglePDF(examId, remarque, note) {
      ...
      await new Promise(resolve => {
        img.onload = () => {
          ...
          // Draw logo on the left in header
          const logoWidth = 40;
          const logoHeight = 40;
          pdf.addImage(img, 'JPEG', 10, 5, logoWidth, logoHeight);

          // Add title in header
          pdf.setTextColor(255, 255, 255);
          pdf.setFontSize(20);
          pdf.text('Correction Details', 60, 25);

          // Reset text color for body
          pdf.setTextColor(0, 0, 0);

          // Draw box for Remarque
          pdf.setDrawColor(52, 152, 219);
          pdf.setLineWidth(0.5);
          pdf.rect(10, 60, pageWidth - 20, 40);

          pdf.setFontSize(14);
          pdf.text('Remarque:', 15, 75);
          pdf.setFontSize(12);
          pdf.text(remarque, 15, 85, { maxWidth: pageWidth - 30 });

          // Draw box for Note
          pdf.rect(10, 110, pageWidth - 20, 30);

          pdf.setFontSize(14);
          pdf.text('Note:', 15, 125);
          pdf.setFontSize(12);
          pdf.text(note, 15, 135, { maxWidth: pageWidth - 30 });

          // Add footer with date and exam ID
          pdf.setFontSize(10);
          pdf.setTextColor(100);
          const dateStr = new Date().toLocaleDateString();
          pdf.text(`Date: ${dateStr}`, 10, pageHeight - 20);
          pdf.text(`Exam ID: ${examId}`, pageWidth - 60, pageHeight - 20);

          // Save PDF
          pdf.save('correction_exam_' + examId + '_' + new Date().toISOString().slice(0, 10) + '.pdf');
          resolve();
        };
